...
class HMI(I2cLcd):
    ...
    def isr_setup(self, pin):
        ...
        if self.pcf.pin(self.button_up)==0:
            ...
            if self.parameter == "drop_factor":
                ...
                self.drop_factor = self.drop_factor + 5
                self.lcd.putstr(str(int(self.drop_factor)))
                if self.drop_factor > 60:
                    self.drop_factor = 60;
                    
            if self.parameter == "volume":
                self.lcd.move_to(7, 2)
                self.volume = self.volume + 50
                if self.volume > 1000.0:
                    self.volume = 1000
    ...
    def screen_setup(self):
        ...
        self.lcd.putstr("Drop factor "+str(self.drop_factor))
