...
def servo_angle(index) -> float:
    return ((servo.degrees/(servo.max_duty - servo.min_duty))*(servo.position(index)-servo.min_duty)) # * 90 /103 ## calibration
...
def control_servo(mark, frequency):
    ...
    if mark > frequency:
        ...
        angle = (servo_angle(0)) - 7
    if angle < 90:
        angle = 90
    servo.position(0, degrees = (angle)) # degrees = ((angle*103)/90) ## calibration
...
def idle():
    ...
    hmi.time_left = (hmi.volume * hmi.drop_factor) /  drip_rate
    hmi.screen_display()
    while True:
